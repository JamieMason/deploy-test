...
module.exports = {
  ...
  // checked/synced. See https://jamiemason.github.io/deploy-test/config/custom-types for more details.
  customTypes: {
    engines: {
      path: "engines",
   ...
   * `deploy-test lint-semver-ranges`, the output is grouped by label.
   */
  semverGroups: [
    ...
    {
      label:
        "Version compatibility workarounds should be used, or removed from deploy-test.config.cjs if no longer needed.",
   ...
   * `deploy-test list-mismatches`, the output is grouped by label.
   */
  versionGroups: [
    // Workaround for compatibility issues.
    // Ideally this section would be empty (and removed).
    ...
    {
      label:
        "Ignore interdependencies on other Fluid packages. This is needed because deploy-test doesn't understand our >= < semver ranges",
      isIgnored: true,
      packages: [
        "@fluid-example/**",
        "@fluid-experimental/**",
