...
deploy-test is a Rust binary crate that creates a command line application for ensuring consistency in the contents of multiple package.json files, particularly focusing on dependency versions.

It is deployed to the npm registry as `deploy-test` in the version range `deploy-test@14.0.0-alpha.*`. It is an in-development replacement for `deploy-test@latest` which is currently on version `13.0.4`.
...
- The `./fixtures/fluid-framework` directory is an example project which can be used to run the local development version of deploy-test against for testing locally.
- The `./npm` directory contains files used when deploying deploy-test to npm, a rust binary for each major OS needs publishing as npm packages which are then set as optionalDependencies of the main deploy-test package, which then has a small node.js script to run the appropriate binary. This is not needed during local development.

## Documentation website

The source code for the documentation website is located in `./site/src/**`, the sitemap for the published website is located at https://jamiemason.github.io/deploy-test/sitemap.xml. Read this sitemap to find what documentation is available to help you with a given topic.
...
- `just benchmark` - When making performance improvements, run this command before and after each change to compare the performance of the current version of deploy-test with the previous version.
...
## Running deploy-test locally

When deployed and installed globally, the end user help documentation for deploy-test as a whole can be found by running:

```bash
deploy-test --help
```

The equivalent command when running a local development version of deploy-test is:
...
- Integration tests use the builder pattern in `src/test/builder.rs` via the `TestBuilder` struct. The `TestBuilder` struct provides a fluent API for creating test cases that consist of package.json files, deploy-test configuration files, and command line inputs.
...
Every deploy-test command follows the same pattern:
