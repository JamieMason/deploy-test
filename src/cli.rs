...
fn create() -> Command {
  Command::new(crate_name!())
    ...
    .subcommand(
      Command::new("lint")
        .about("Lint all versions and ranges and exit with 0 or 1 based on whether all files match your deploy-test configuration file")
...
<dim>$</dim> <blue><bold>deploy-test {command}</bold> --dependencies 'react'</>
<dim>Substring match for "react"</>
<dim>$</dim> <blue><bold>deploy-test {command}</bold> --dependencies '**react**'</>
<dim>All dependencies under the AWS SDK scope</>
<dim>$</dim> <blue><bold>deploy-test {command}</bold> --dependencies '@aws-sdk/**'</>
<dim>Exact match for "react" or "webpack" (2 approaches)</>
<dim>$</dim> <blue><bold>deploy-test {command}</bold> --dependencies 'react' --dependencies 'webpack'</>
<dim>$</dim> <blue><bold>deploy-test {command}</bold> --dependencies '{has_braces}'</>
...
<dim>$</dim> <blue><bold>deploy-test {command}</bold> --show statuses</>
<dim>Show all instances, not just their names</dim>
<dim>$</dim> <blue><bold>deploy-test {command}</bold> --show instances</>
<dim>Show highest level of detail</dim>
<dim>$</dim> <blue><bold>deploy-test {command}</bold> --show all</>
<dim>Show lowest level of detail</dim>
<dim>$</dim> <blue><bold>deploy-test {command}</bold> --show none</>"#
    ))
    .value_delimiter(',')
...
<dim>$</dim> <blue><bold>deploy-test {command}</bold> --show ignored</>
<dim>Show highest level of detail</dim>
<dim>$</dim> <blue><bold>deploy-test {command}</bold> --show all</>
<dim>Choose only some values</dim>
<dim>$</dim> <blue><bold>deploy-test {command}</bold> --show hints,statuses</>
<dim>Show lowest level of detail</dim>
<dim>$</dim> <blue><bold>deploy-test {command}</bold> --show none</>"#
    ))
    .value_delimiter(',')
...
<dim>$</dim> <blue><bold>deploy-test {command}</bold> --sort count</>
<dim>Sort A-Z by name</>
<dim>$</dim> <blue><bold>deploy-test {command}</bold> --sort name</>"#
    ))
...
<dim>$</dim> <blue><bold>deploy-test {command}</bold> --show instances --specifier-types exact
<dim>Missing or unsupported versions</>
<dim>$</dim> <blue><bold>deploy-test {command}</bold> --show instances --specifier-types missing,unsupported
<dim>Latest or workspace protocol only</>
<dim>$</dim> <blue><bold>deploy-test {command}</bold> --show instances --specifier-types latest,workspace-protocol"#
    ))
    .value_delimiter(',')
    .value_parser([
      "alias",
...
See <blue>https://jamiemason.github.io/deploy-test/config/custom-types/</>

<bold><underline>Examples:</underline></bold>
<dim>devDependencies only</>
<dim>$</dim> <blue><bold>deploy-test {command}</bold> --dependency-types dev
<dim>dependencies and devDependencies only</>
<dim>$</dim> <blue><bold>deploy-test {command}</bold> --dependency-types dev,prod"#
    ))
    .value_delimiter(',')
...
<bold><underline>Examples:</underline></bold>
<dim>$</dim> <blue><bold>deploy-test {command}</bold> --dry-run</>"#
    ))
    .action(clap::ArgAction::SetTrue)
}
...
<dim>$</dim> <blue><bold>deploy-test {command}</bold> --log-levels off</>
<dim>Only show verbose debugging logs</dim>
<dim>$</dim> <blue><bold>deploy-test {command}</bold> --log-levels debug</>
<dim>Show everything</dim>
<dim>$</dim> <blue><bold>deploy-test {command}</bold> --log-levels error,warn,info,debug</>"#
    ))
    .value_delimiter(',')
...
<bold><underline>Examples:</underline></bold>
<dim>$</dim> <blue><bold>deploy-test {command}</bold> --no-ansi</>"#
    ))
    .action(clap::ArgAction::SetTrue)
}
...
<bold><underline>Examples:</underline></bold>
<dim>$</dim> <blue><bold>deploy-test {command}</bold> --source 'package.json' --source 'apps/*/package.json'</>
...
2. <blue>.source</> property of your deploy-test config file
...
<dim>$</dim> <blue><bold>deploy-test {command}</bold> --target latest</>
<dim>Only update minor versions (1.x.x)</dim>
<dim>$</dim> <blue><bold>deploy-test {command}</bold> --target minor</>
<dim>Only update patch versions (1.2.x)</dim>
<dim>$</dim> <blue><bold>deploy-test {command}</bold> --target patch</>"#
    ))
    .action(clap::ArgAction::Set)
...
- Documentation: <blue><underline>https://jamiemason.github.io/deploy-test</></>"#
  )
}
