...
pub fn try_from_package_json_config_property(cli: &Cli) -> Option<Result<Rcfile, RcfileError>> {
  ...
  package_json_path
    ...
    .and_then(|mut package_json| {
      package_json
        .get_mut("deploy-test")
        .inspect(|_| debug!("Found .deploy-test property in package.json"))
        .map(|config| config.take())
        .or_else(|| {
          package_json
            .pointer_mut("/config/deploy-test")
            .inspect(|_| debug!("Found .config.deploy-test property in package.json"))
            .map(|config| config.take())
        })
